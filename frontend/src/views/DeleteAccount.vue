...
async function deleteAccount(){ if(!form.emailCode){alert('请输入邮箱验证码');return} if(!confirm('确认注销账号？该操作会退出全部设备。')) return; loading.value=true; try{ const res=await requestDeleteAccountApi(form); alert(res.message||'账号已进入待注销状态'); removeAuthTokens(); router.push('/login') } finally{ loading.value=false } }
</script>
